...
public:
  PrototypeAST(const std::string &Name, std::vector<std::string> Args,
               bool IsOperator = false, unsigned Precendence = 0)
...
// Parsing fundctions to construct AST
...
// Helper function to create an alloca instruction in the entry block of the
// function This is used for mutable variables etc.
static AllocaInst *CreateEntryBlockAlloca(Function *TheFunction,
                                          const std::string &VarName) {
  IRBuilder<> TmpBuilder(&TheFunction->getEntryBlock(),
                         TheFunction->getEntryBlock().begin());
  return TmpBuilder.CreateAlloca(Type::getDoubleTy(TheContext), 0,
                                 VarName.c_str());
}
...
Value *BinaryExprAST::codegen() {
  // Special case for '=' because we don't want to emit the LHS as an expression
  if (Op == '=') {
...
Value *IfExprAST::codegen() {
  ...
  // The then expression may change the the current block, we need to get the
  // current block again for the phi node
  ...
  if (!ElseV)
    return nullptr;
  Builder->CreateBr(MergeBB);
  // The else expression may change the the current block, we need to get the
  // current block again for the phi node
...
Value *ForExprAST::codegen() {
  ...
  // Store the value into the allocca
...
Function *PrototypeAST::codegen() {
  ...
  // for a given Prototype. False indicates not vararg
  FunctionType *FT =
      FunctionType::get(Type::getDoubleTy(TheContext), Doubles, false);

  // Creates the IR function corresponding to the Prototype.
  // "external linkage" means that the function may be found outside the current
...
Function *FunctionAST::codegen() {
  ...
  for (auto &Arg : TheFunction->args()) {
    // Create storage for the argument
    AllocaInst *Alloca = CreateEntryBlockAlloca(TheFunction, Arg.getName());
    // Store the value of the argument
    Builder->CreateStore(&Arg, Alloca);
    // Map argument name to the memory location
...
Value *VarExprAST::codegen() {
  ...
  for (i = 0; i < e; i++) {
    ...
    // Store the old variable bindings
...
static void HandleTopLevelExpression() {
  ...
  if (auto FnAST = ParseTopLevelExpr()) {
    if (auto *FnIR = FnAST->codegen()) {
      ...
      // Get the symbol's address and cast it to a function pointer so that we
...
“dlsym("fun")” on the Kaleidoscope process itself.
*/
